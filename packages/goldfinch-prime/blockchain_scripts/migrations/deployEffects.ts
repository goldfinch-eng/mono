...
const GNOSIS_CONTRACT_NETWORK_CONFIG_BY_CHAIN_ID: Record<number, ContractNetworkConfig> = {
  // Found here: https://github.com/safe-global/safe-contracts/blob/e870f514ad34cd9654c72174d6d4a839e3c6639f/CHANGELOG.md#expected-addresses-with-safe-singleton-factory-1
  1: {
    // mainnet
...
function getMultisendContractForChainId(chainId: number): {address: string; abi: AbiItem[]} {
  const maybeConfig = GNOSIS_CONTRACT_NETWORK_CONFIG_BY_CHAIN_ID[chainId]

  if (maybeConfig === undefined) {
    throw new Error(`Gnosis contracts not registered for network ${chainId}`)
  }

  return {
    address: maybeConfig.multiSendAddress,
...
class DefenderMultisendEffects extends MultisendEffects {
  ...
  async execute(safeTxs: SafeTransactionDataPartial[], params?: {dryRun?: boolean}): Promise<void> {
    ...
    await defender.send({
      method: "multiSend",
      contract: getMultisendContractForChainId(Number(this.chainId)),
      args: [multisendData],
...
async function constructSafe(params: {via?: string; executor: string}): Promise<Safe> {
  ...
  const safe = await Safe.create({
    ethAdapter,
    safeAddress: via,
    contractNetworks: GNOSIS_CONTRACT_NETWORK_CONFIG_BY_CHAIN_ID,
  })
  return safe
}

export type DeployEffectsParams = {
  via?: string
  title?: string
  description?: string
  safeConfig?: {safeAddress: string; executor: string}
...
export async function getDeployEffects(params?: DeployEffectsParams): Promise<DeployEffects> {
  ...
  if (isMainnetForking()) {
    try {
      ...
      console.log(`Mainnet forking environment was not able to get a Gnosis Safe. Encountered error: ${e}`)
      console.log("Falling back to IndividualTxEffects")
