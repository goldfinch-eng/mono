...
describe("PoolTokens", () => {
  ...
  describe("mint", async () => {
    beforeEach(async function () {
      ...
      pool = await getTruffleContractAtAddress("TestTranchedPool", deployments.poolAddress)
  ...
  describe("withdrawPrincipal", async () => {
    ...
    beforeEach(async () => {
      const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(`PoolTokens`)({
        usdcAddress: usdc.address,
        borrower: person2,
        juniorFeePercent: new BN(20),
        limit: usdcVal(100),
  ...
  describe("redeem", async () => {
    ...
    beforeEach(async function () {
      const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(`PoolTokens redeem`)({
        usdcAddress: usdc.address,
        borrower: person2,
        juniorFeePercent: new BN(20),
        limit: usdcVal(100),
  ...
  describe("burning", async () => {
    let tokenId, mintAmount
    beforeEach(async function () {
      const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(`PoolTokens burning`)({
        usdcAddress: usdc.address,
        borrower: person2,
        juniorFeePercent: new BN(20),
        limit: usdcVal(100),
  ...
  describe("go listing", async () => {
    let amount
    beforeEach(async function () {
      const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(`PoolTokens go listing`)({
        usdcAddress: usdc.address,
        borrower: person2,
        juniorFeePercent: new BN(20),
        limit: usdcVal(100),
  ...
  describe("setBaseURI", async () => {
    ...
    it("sets base URI for tokenURI(tokenId) calls", async () => {
      ...
      const event = decodeAndGetFirstLog<PoolCreated>(result.receipt.rawLogs, goldfinchFactory, "PoolCreated")
      pool = await getTruffleContractAtAddress("TestTranchedPool", event.args.pool)
