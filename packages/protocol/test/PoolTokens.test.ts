...
import {OWNER_ROLE, interestAprAsBN, GO_LISTER_ROLE, POOL_VERSIONS} from "../blockchain_scripts/deployHelpers"
...
import {
  ...
  deployUninitializedV2TranchedPoolFixture,
} from "./util/fixtures"
...
describe("PoolTokens", () => {
  ...
  POOL_VERSIONS.forEach((poolVersion) => {
    describe(`TranchedPool ${poolVersion}`, () => {
      describe("mint", async () => {
        beforeEach(async function () {
          const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture("PoolTokens mint")({
          ...
          pool = await TranchedPool.at(deployments.poolAddress)
        ...
        context("with real pool validation turned on", async () => {
          ...
          it("should disallow invalidly created pools", async () => {
            ...
            const {tranchedPool: fakePool} =
              poolVersion[0] === "0"
                ? await deployUninitializedTranchedPoolFixture()
                : await deployUninitializedV2TranchedPoolFixture()
      ...
      describe("withdrawPrincipal", async () => {
        ...
        beforeEach(async () => {
          const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(`PoolTokens ${poolVersion}`)({
            usdcAddress: usdc.address,
            borrower: person2,
            juniorFeePercent: new BN(20),
            limit: usdcVal(100),
      ...
      describe("redeem", async () => {
        ...
        beforeEach(async function () {
          const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(`PoolTokens redeem ${poolVersion}`)({
            usdcAddress: usdc.address,
            borrower: person2,
            juniorFeePercent: new BN(20),
            limit: usdcVal(100),
          ...
          let result = await pool.deposit(new BN(1), mintAmountA, {from: person2})
      ...
      describe("reducePrincipalAmount", async () => {
        ...
        beforeEach(async function () {
          const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(
            `PoolTokens reducePrincipalAmount ${poolTokens}`
          )({
            usdcAddress: usdc.address,
            borrower: person2,
            juniorFeePercent: new BN(20),
          ...
          const result = await pool.deposit(new BN(2), mintAmount, {from: person2})
        ...
        describe("after partially redeeming", async () => {
          ...
          describe("as a wallet without OWNER_ROLE", () => {
            it("it fails", async () => {
              await expect(
                poolTokens.reducePrincipalAmount(tokenId, redemptionAmount, {from: person2})
      ...
      describe("burning", async () => {
        let tokenId, mintAmount
        beforeEach(async function () {
          const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(`PoolTokens burning ${poolVersion}`)({
            usdcAddress: usdc.address,
            borrower: person2,
            juniorFeePercent: new BN(20),
            limit: usdcVal(100),
          ...
          mintAmount = usdcVal(5)
          const result = await pool.deposit(new BN(1), mintAmount, {from: person2})
        ...
        it("should disallow burning if the token isn't fully redeemed", async () => {
          return expect(withPoolSender(() => poolTokens.burn(tokenId))).to.be.rejectedWith(
            /Can only burn fully redeemed/
          )
        })
      ...
      describe("go listing", async () => {
        ...
        beforeEach(async function () {
          const deployments = await deployTranchedPoolWithGoldfinchFactoryFixture(
            `PoolTokens go listing ${poolVersion}`
          )({
            usdcAddress: usdc.address,
            borrower: person2,
            juniorFeePercent: new BN(20),
