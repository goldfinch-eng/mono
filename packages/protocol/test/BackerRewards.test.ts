...
describe("BackerRewards", function () {
  ...
  let owner: string,
    ...
    tranchedPool: TranchedPoolInstance | TranchedPoolV2Instance,
    creditLine: CreditLineInstance | CreditLineV2Instance,
  ...
  async function pay(pool: TranchedPoolInstance | TranchedPoolV2Instance, amount: BN) {
  ...
  const testSetup = (poolVersion: string) =>
    deployments.createFixture(async () => {
      ...
      ;({tranchedPool, creditLine} = await getTranchedPoolAndCreditLine(poolAddress, clAddress, poolVersion))
  ...
  describe("v0.1.0 only tests", () => {
    ...
    describe("perverse scenario in which borrower would drawdown interest they'd previously repaid, to get duplicative rewards from their next repayment", () => {
      ...
      // of rewards.
      let pool: TranchedPoolInstance | TranchedPoolV2Instance
      let cl: CreditLineInstance | CreditLineV2Instance
      const paymentPeriodInDays = new BN(30)
      ...
      beforeEach(async () => {
        ...
        ;({tranchedPool: pool, creditLine: cl} = await getTranchedPoolAndCreditLine(
          poolAddress,
          clAddress,
          POOL_VERSION1
        ))
  ...
  describe("v1.0.0 only tests", () => {
    beforeEach(async () => {
      ...
      ;({
        ...
        tranchedPool,
        creditLine,
      } = await testSetup("1.0.0")())
    })

    describe("perverse scenario in which borrower would drawdown interest they'd previously repaid, to get duplicative rewards from their next repayment", () => {
      let pool: TranchedPoolV2Instance
      let cl: TestCreditLineV2Instance
      const paymentPeriodInDays = new BN(30)
      ...
      beforeEach(async () => {
        ...
        const contracts = await getTranchedPoolAndCreditLine(poolAddress, clAddress, POOL_VERSION2)
        pool = contracts.tranchedPool as TranchedPoolV2Instance
        cl = contracts.creditLine as TestCreditLineV2Instance
      })
  ...
  POOL_VERSIONS.forEach((version) => {
    describe(`TranchedPool version ${version}`, () => {
      beforeEach(async () => {
        // eslint-disable-next-line @typescript-eslint/no-extra-semi
        ;({
          owner,
      ...
      describe("allocateRewards()", () => {
        ...
        context("Should not execute if interestPayment is 0", () => {
          ...
          it("should be fulfilled, not error", async () => {
            await expect(
              withPoolSender(() => backerRewards.allocateRewards(interestPaymentAmount), tranchedPool.address)
            ).to.be.fulfilled
          })
        })
      })
      ...
      describe("withdraw()", () => {
        ...
        it("rejects senior-tranche token", async () => {
          ...
          const payAmount = usdcVal(totalPrincipal).add(usdcVal(5_000))
          await erc20Approve(usdc, tranchedPool.address, payAmount, [borrower])
          await advanceTime({toSecond: await creditLine.termEndTime()})
          await pay(tranchedPool, payAmount)
        ...
        context("withdrawMultiple()", () => {
          it("successfully updates claimed amount and transfers gfi for multiple pooltokens", async () => {
            ...
            await expect(backerRewards.withdrawMultiple([anotherUser2TokenId, anotherUserTokenId], {from: anotherUser}))
              .to.be.fulfilled
      ...
      describe("tranchedPool interest repayment", () => {
        ...
        maxRepaymentTestCases.forEach(
          async ({
            ...
            it(`should handle a MAX 100% apy full repayment totalRewards:${totalRewards}, totalGFISupply:${totalGFISupply}`, async () => {
              ...
              const {tranchedPool: tranchedPoolMax} = await getTranchedPoolAndCreditLine(
                poolAddress,
                clAddress,
                version
              )
        ...
        describe("perverse scenario regarding total junior deposits", () => {
          ...
          beforeEach(async () => {
            ...
            const contracts = await getTranchedPoolAndCreditLine(poolAddress, clAddress, POOL_VERSION1)
      ...
      context(`Changing rewards or total supply gfi ${version}`, () => {
        it("changing total rewards or max interest dollars after interest has been received", async () => {
          ...
          const {tranchedPool: tranchedPoolMax} = await getTranchedPoolAndCreditLine(poolAddress, clAddress, version)
      ...
      describe(`Staking-rewards-related view functions ${version}`, () => {
        const maxInterestDollarsEligible = 1_000_000_000
        const totalGFISupply = 100_000_000
        ...
        describe("stakingRewardsClaimed", () => {
          it("Junior-tranche pool token returns non-0 as appropriate", async () => {
            ...
            const juniorStakingRewardsEarnedAfter = await backerRewards.stakingRewardsEarnedSinceLastWithdraw(
              juniorTokenId
            )
