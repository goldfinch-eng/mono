...
import {SeniorPool} from "../graphql/types"
...
function PoolStatus({poolData}: PoolStatusProps) {
  ...
  function deriveRows() {
    ...
    if (poolData) {
      defaultRate = poolData.lastestPoolStatus?.defaultRate
      poolBalance = usdcFromAtomic(poolData.lastestPoolStatus?.totalPoolAssets)
      totalLoansOutstanding = usdcFromAtomic(poolData.lastestPoolStatus?.totalLoansOutstanding)
      capacityRemaining = poolData.lastestPoolStatus?.remainingCapacity(maxPoolCapacity)
    }
  ...
  return (
    <div className={`pool-status background-container ${poolData ? "" : "placeholder"}`}>
      <h2>Pool Status</h2>
