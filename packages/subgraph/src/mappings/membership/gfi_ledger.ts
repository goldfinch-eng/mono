...
export function handleGfiWithdraw(event: GFIWithdrawal): void {
  if (event.params.remainingAmount.isZero()) {
    ...
    const vaultedGfi = assert(VaultedGfi.load(event.params.positionId.toString()))
    vaultedGfi.amount = event.params.remainingAmount
    vaultedGfi.save()
  }

  const transaction = createTransactionFromEvent(event, "MEMBERSHIP_GFI_WITHDRAWAL", event.params.owner)
