...
export function handleIncreasedHoldings(event: IncreasedHoldings): void {
  const membership = getOrInitMembership(event.params.owner)
  membership.nextEpochScore = event.params.amount
  membership.save()
...
export function handleDecreasedHoldings(event: DecreasedHoldings): void {
  const membership = getOrInitMembership(event.params.owner)
