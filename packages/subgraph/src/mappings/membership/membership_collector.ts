...
import {Membership, MembershipEpoch, MembershipRewardDisbursement} from "../../../generated/schema"
...
export function handleEpochFinalized(event: EpochFinalized): void {
  ...
  for (let i = 0; i < membershipRoster.members.length; i++) {
    ...
    const disbursement = new MembershipRewardDisbursement(`${event.params.epoch.toString()}-${membership.id}`)
