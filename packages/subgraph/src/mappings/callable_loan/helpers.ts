...
export function getOrInitCallableLoan(address: Address): CallableLoan {
  const id = address.toHexString()
  let callableLoan = CallableLoan.load(id)
  if (!callableLoan) {
    callableLoan = new CallableLoan(id)
    ...
    callableLoan.createdAt = 0 // This has to be filled in when the pool is first created by GoldfinchFactory
