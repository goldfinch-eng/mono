...
import {JsonAccountedGrant, MerkleDistributorGrantInfo} from "../../../blockchain_scripts/merkleDistributor/types"
...
describe("generateMerkleRoot", () => {
  ...
  it("generates the Merkle root, amount total, and grant details of the rewards distribution", async () => {
    ...
    const json: JsonAccountedGrant[] = [accountedGrant0, accountedGrant1, accountedGrant2]
    const merkleDistributorInfo = generateMerkleRoot(json)

    const expectedGrants: MerkleDistributorGrantInfo[] = [
      {
        index: 0,
    ...
    expect(merkleDistributorInfo).to.eql({
