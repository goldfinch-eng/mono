...
async function main(bre) {
  if (!process.env.SHOULD_UPGRADE) {
    bre.deployments.log("Not in upgrade mode, so skipping the upgrade script. If you want this to run set SHOULD_UPGRADE as an env var at the beginning of your script call");
    return
  }
  bre.deployments.log("Starting the upgrade...");
  await baseDeploy(bre, {shouldUpgrade: true});
};
