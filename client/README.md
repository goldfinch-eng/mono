...
## Developing
- Run a local blockchain. `npx buidler node` from the project root directory.
- Run the server. `npm start` from the `client` directory.
...
environment. To really take advantage of this, you should login to your Metamask first, then copy your account number, and paste it in like the following:
  - `TEST_USER={YOUR_METAMASK_ADDRESS} npx buidler node --export-all ./config/deployments.json ` from the project root directory.
