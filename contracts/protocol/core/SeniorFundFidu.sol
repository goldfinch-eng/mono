 ...
 *  in the SeniorFund. When you deposit, we mint a corresponding amount of Fidu, and when you withdraw, we
 *  burn Fidu. The share price of the SeniorFund implicitly represents the "exchange rate" between Fidu
 *  and USDC (or whatever currencies the SeniorFund may allow withdraws in during the future)
 * @author Goldfinch
 */

contract SeniorFundFidu is ERC20PresetMinterPauserUpgradeSafe {
  ...
  */
  ...
  // canMint assumes that the USDC that backs the new shares has already been sent to the SeniorFund
  function canMint(uint256 newAmount) internal view returns (bool) {
    IFund fund = config.getSeniorFund();
  ...
  // canBurn assumes that the USDC that backed these shares has already been moved out the SeniorFund
